      ...
      <table class="common_table">
        ...
        {foreach from=$invoice_item_templates item=predefined_item}
          <tr class="{cycle values='odd,even'} template" id="item_template_{$predefined_item->getId()}">
            ...
            <td class="unit_cost">{$predefined_item->getUnitCost()|number:$smarty.const.INVOICE_PRECISION|clean}</td>
            <td class="quantity">{$predefined_item->getQuantity()|number:$smarty.const.INVOICE_PRECISION|clean}</td>       
            <td class="tax">
