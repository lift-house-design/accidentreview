  ...
  <div class="object_list">
    ...
    {if is_foreachable($invoices)}
      ...
      {foreach from=$invoices item=invoice}
        ...
        {if $invoice->isIssued()}
          <td class="status">{action_on_by datetime=$invoice->getIssuedOn() user=$invoice->getIssuedBy() action='Issued' format=date offset=0}</td>
        ...
        {elseif $invoice->isBilled()}
          <td class="status">{action_on_by datetime=$invoice->getClosedOn() user=$invoice->getClosedBy() action='Billed' format=date offset=0}</td>
          <td class="due_on no_due_date">--</td>
          <td class="pdf dont_print"><a href="{$invoice->getCompanyPdfUrl()}" title="{lang}Download Invoice in PDF Format{/lang}"><img src="{image_url name=pdf-small.gif}" alt="" /></a></td>
        {elseif $invoice->isCanceled()}
          <td class="status">{action_on_by datetime=$invoice->getClosedOn() user=$invoice->getClosedBy() action='Canceled' format=date offset=0}</td>
          <td class="due_on no_due_date">--</td>
