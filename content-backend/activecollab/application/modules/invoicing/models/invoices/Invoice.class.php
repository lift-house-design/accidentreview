<?php
  ...
  class Invoice extends BaseInvoice {
      ...
      if($cache == false || $this->total === false || $this->tax === false || $this->taxed_total === false) {
        ...
        if(is_foreachable($this->getItems())) {
          foreach($this->getItems() as $item) {
            ...
            $this->tax   = $this->tax + $item->getTax();
          } // foreach
        } // if
        
        $this->taxed_total = $this->total + $this->tax;
        
        $this->total = round($this->total * INVOICE_PRECISION_ROUNDER) / INVOICE_PRECISION_ROUNDER;
        $this->tax   = round($this->tax * INVOICE_PRECISION_ROUNDER) / INVOICE_PRECISION_ROUNDER;
        $this->taxed_total   = round($this->taxed_total * INVOICE_PRECISION_ROUNDER) / INVOICE_PRECISION_ROUNDER;
      } // if
    } // calculateTotal
    ...
    function getMaxPayment($cache = true) {
      return round(($this->getTaxedTotal($cache) - $this->getPaidAmount($cache)) * INVOICE_PRECISION_ROUNDER) / INVOICE_PRECISION_ROUNDER;
    } // getMaxPayment
