<?php
  ...
  class Invoices extends BaseInvoices {
    ...
    /**
     * Filters timerecords and leave only ones who are not assigned to any invoice
     *  - returns true if there was some timerecords that are removed
     * 
     * @param array
     * @return boolean
     */
    function filterAvailableTimerecords(&$timerecord_ids) {
      $invoice_time_records_table = TABLE_PREFIX . 'invoice_time_records';
      // find timerecords that are already attached to some invoice and remove them from the list of timerecords that
      // will be attached to this invoice
      $existing_timerecord_ids = db_execute_all("SELECT time_record_id FROM $invoice_time_records_table WHERE time_record_id IN (?)", $timerecord_ids);
      if (is_foreachable($existing_timerecord_ids)) {
        foreach ($existing_timerecord_ids as $existing_timerecord_id) {
        	$key = array_search($existing_timerecord_id['time_record_id'], $timerecord_ids);
        	if ($key !== null) {
        	  unset($timerecord_ids[$key]);
        	} // if
        } // foreach
      } // if
      // fix array keys
      $timerecord_ids = array_values($timerecord_ids);
      
      return is_foreachable($existing_timerecord_ids);
    } // filterAvailableTimerecords
