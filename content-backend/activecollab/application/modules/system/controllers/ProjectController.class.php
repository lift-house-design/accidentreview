...
//        } // if
    ...
    function add() {
      ...
      if($this->request->isSubmitted()) {
        ...
        /**
        * Custom Project / Company Creation Process
        */
        ...
        $user_emails = $this->request->post('company_user_email');
        $user_first_names = $this->request->post('company_user_first');
        $user_last_names = $this->request->post('company_user_last');
        $user_passwords = $this->request->post('company_user_password');
        $user_admins = $this->request->post('company_user_admin');
        
        $company_users = array();
        
        $company_errors = array();
        foreach($user_emails as $key=>$an_email){
            if($an_email != ''
                && $user_first_names[$key] != ''
                && $user_last_names[$key] != ''
                && $user_passwords[$key] != '' ){
                    
                    $existing = false;
                    
                    $find_s = 'SELECT id FROM acx_users WHERE email=\''.$an_email.'\'';
                    $find_q = mysql_query($find_s,$dbh);
                    if($find_q !== false && count($find_q) > 0){
                        $found = false;
                        while ($row = mysql_fetch_assoc($find_q)) if($row['email'] == $an_email) $found = true;
                    ...
                    if(!$existing){
                        ...
                        $company_users[] = array(
                            'email' => $an_email,
                            'first_name' => $user_first_names[$key],
                            'last_name' => $user_last_names[$key],
                            'password' => sha1(LICENSE_KEY . $user_passwords[$key]),
                            'password_plain' => $user_passwords[$key],
                            'admin' => in_array(($key + 1),$user_admins)? 1:0
                        );
                    } else {
                        $company_errors[] = 'This email ('.$an_email.') is already in use in this system.';
                    }
                } else {
                    $company_errors[] = 'Email, Name, and Password are required for users.';
                }
        }
        
        if(count($company_users) == 0){
            $user_create_error = true;
            $company_errors[] = 'At least one user is required for a company.';            
        }
        
       
        
        $this->smarty->assign('company_users',$company_users);
        ...
        if(!$user_create_error){
            ...
            $new_company->setAttributes($this->request->post('company'));
            $new_company->setName($project_data['name']);
            $new_company->setIsOwner(false);
            ...
            $this->active_project->setCompanyId($company_id);
            
            /* Create Company Users */
            
            $find_s = 'SELECT * FROM acx_roles WHERE `name` LIKE \'%Company%\'';
            $find_q = mysql_query($find_s,$dbh);
            $user_role_member=0;
            $user_role_admin=0;
            $user_role_user=0;
            ...
            while($row = mysql_fetch_assoc($find_q)){
                if($row['name'] == 'Company Admin') $user_role_admin = $row['id'];
                if($row['name'] == 'Company User') $user_role_user = $row['id'];
                if($row['name'] == 'Company Member') $user_role_member = $row['id'];
            }
            
            $role_member = new Role($user_role_member);
            $role_user = new Role($user_role_user);
            $role_admin = new Role($user_role_admin);
                       
            $company_user_objs = array();

            foreach($company_users as $company_user_key=>$a_company_user){
                $user_error_obj = new ValidationErrors();
                $new_user = new User();
                $new_user->setCompanyId($company_id);
                $new_user->setFirstName($a_company_user['first_name']);
                $new_user->setLastName($a_company_user['last_name']);
                $new_user->setEmail($a_company_user['email']);
                $new_user->setPassword($a_company_user['password_plain']);
                $new_user->setRoleId($user_role_member);
                $new_user->setCreatedOn(new DateTimeValue(time()));
                $new_user->setAutoAssign(0);
                $new_user->setAutoAssignRoleId(9);
                
                $new_user->validate($user_error_obj);
                if($user_error_obj->hasErrors()){
                    die($user_error_obj->getErrorsAsString());
                } else {
                    $new_user->save();
                    $company_users[$company_user_key]['obj'] = $new_user;
                }
            }
            
            
        /**
        * End Custom Project / Company Creation Process
        */
            ...
            if($save && !is_error($save)) {
              ...
              /**
               * Add Users to New Project
               */
              foreach($company_users as $a_company_user){
                    if($a_company_user['admin'] == '1'){
                        $results = $this->active_project->addUser($a_company_user['obj'],$role_admin);    
                        if(is_error($results)) die($results->getErrorsAsString());
                    } else {
                        $results = $this->active_project->addUser($a_company_user['obj'],$role_user);
                        if(is_error($results)) die($results->getErrorsAsString());
                    }
              }
              ...
              if($this->request->isApiCall()) {
                ...
                echo 'Save: '.print_r($save,true);
                echo 'User: '.print_r($user_error_obj,true);
                $this->smarty->assign('errors', $save);
              } // if
            } // if
        } else {
            db_rollback();
             $user_save = new ValidationErrors($company_errors);
            if($this->request->isApiCall()) {
                $this->serveData($save);
            } else {
                $this->smarty->assign('errors', $user_save);
            } // if
        }
      } // if
    } // add
