<?php
  ...
  class ProjectObjects extends BaseProjectObjects {
    ...
    /**
     * Find all objects related to a milestone
     * 
     * Only first-level objects are returned and no permission filter are applied because
     * this method is used for copy/move to project feature
     * 
     * If you want user permissions and visibility rules applied, use Milestone::getObjects()
     *
     * @param ProjectObject $milestone
     * @param string $order
     * @return array
     */
    function findByMilestone($milestone, $order = null) {
      return ProjectObjects::find(array(
        'conditions' => "milestone_id = '".$milestone->getId()."' AND type IN ('Ticket', 'Checklist', 'Discussion', 'File')",
        'order' => $order,
      ));
    } // findByMilestone
