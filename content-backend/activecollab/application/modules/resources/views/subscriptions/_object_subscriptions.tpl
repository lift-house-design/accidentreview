{if $_object_subscription_brief}
  <div id="object_subscriptions_for_{$_object_subscriptions_object->getId()}" popup_url={$_object_subscriptions_popup_url}>
    ...
    {if $_object_subscriptions_list_subscribers}
      ...
      {/if}
    {else}
      {if $_object_subscriptions_subscribers_count > 0}
        {assign_var name=person}{if $_object_subscriptions_subscribers_count==1}{lang}person{/lang}{else}{lang}people{/lang}{/if}{/assign_var}
        {lang subscribers_count=$_object_subscriptions_subscribers_count type=$_object_subscriptions_object->getVerboseType(true) person=$person}:subscribers_count :person subscribed to this :type{/lang}
      {else}
        {lang type=$_object_subscriptions_object->getVerboseType(true)}There are no users subscribed to this :type{/lang}
      {/if}
    {/if}
