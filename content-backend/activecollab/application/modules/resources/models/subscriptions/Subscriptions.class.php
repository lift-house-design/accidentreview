  ...
  class Subscriptions extends BaseSubscriptions {
    ...
    function subscribe($user, $object) {
      ...
      if($save && !is_error($save)) {
		...
		///// Added by Nick Niebaum: E-mail if technician subscribed ///// 
		if($object->getTypeName()=='ticket') // If subscribing to a ticket
		{
			if ($user->getRoleId()==12) // and user is a technician
			{
				// Then a technician is being assigned to a ticket... e-mail notifications
				require_once('/var/www/vhosts/accidentreview.com/content-backend/custom/email_funcs.php');
				
				// [ NOTIFY THE INSURANCE USER ]
				$ticket_id=$object->getId();
				$to_name=$object->getCreatedByName();
				$to_email=$object->getCreatedByEmail();
				$template_name='tech_assigned_for_user';
				$subject_vars=array(
					':object_type'=>'assignment',
				);
				
				$tech_name=trim($user->getFirstName().' '.$user->getLastName());
				if(empty($tech_name))
				{
				...
				ac_email($ticket_id,$to_name,$to_email,$template_name,$subject_vars,$body_vars);
			}
		}
		///// /Added by /////
