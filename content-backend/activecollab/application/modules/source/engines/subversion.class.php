<?php
  ...
  class RepositoryEngine extends Repository {
    ...
    /**
     * Trust server certificate (subversion >= v1.6)
     *
     * @var string
     */
    var $trust_server_certificate = '';
    ...
    function getHeadRevision($isAsync = false) {
      ...
      if (isset($info['revision']) && $info['revision'] !== false) {
        ...
        $error_message = lang('Could not obtain the highest revision number for the given repository');
    ...
    /**
     * Get first revision for repository
     * 
     * This is useful for getting commit history for repository subdirectories, because
     * they don't start from r1
     *
     */
    function getFirstRevision($isAsync = false) {
      $this->triggerred_by_handler = $isAsync;
      
      $string = 'log -r 1:HEAD --limit 1 --xml '.$this->getUrl();
      $this->execute($string);
      ...
      if (isset($log_data['0'])) {
        $first_revision = $log_data['0']['attr']['revision'];
      } // if
    ...
    function checkResponse($response) {
      ...
      if (is_foreachable($response)) {
        ...
        elseif (strpos($response['0'], 'Repository moved permanently')) {
          return new Error($response['0']);
        }
        elseif (strpos($response['0'], "Can't connect to host")) {
          return new Error($response['0']);
        }
        elseif (strpos($response['0'], 'File not found')) {
          return new Error($response['0']);
        }
        elseif (strpos($response['0'], 'authorization failed')) {
          return new Error($response['0']);
        }
        ...
        elseif (strpos($response['0'], 'Not a valid URL')) {
          return new Error($response['0']);
        }
        elseif (strpos($response['0'], 'path not found')) {
          return new Error($response['0']);
        }
        elseif (strpos($response['0'], 'refers to a directory')) {
          return new Error(lang('Selected item is a directory'));
        }
        elseif (isset($response['1']) && strpos($response['1'], 'Host not found')) {
          return new Error(lang("Repository hostname not found"));
        }
        elseif (strpos($response['1'], 'could not connect')) {
          return new Error($response['1']);
        }
        elseif (isset($response['2']) && strpos($response['2'], 'authorization failed')) {
          return new Error(lang("Authorization failed"));
        }
        elseif (strpos($response['0'], 'Username not recognized')) {
          return new Error($response['0']);
        }
        elseif (strpos($response['0'], 'SSL not supported')) {
          return new Error($response['0']);
        }
    ...
    function executableExists($path = null) {
      ...
      exec(escapeshellcmd($svn_path . 'svn '.$this->config_dir.' --version --quiet'). " ".$this->stderr2stdout, $output);
      $output = first($output);
     ...
     * @return string
     */
    function getRepositoryRootPath() {
      if (is_null($this->root_path)) {
        $this->execute('info '.$this->active_repository->getUrl());
        $this->root_path = str_replace('Repository Root: ', '', $this->output['2']);
      } // if
